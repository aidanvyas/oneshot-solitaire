...
class TestOnePassSolitaireRules(unittest.TestCase):
    ...
    def test_valid_tableau_move_alternating_descending(self):
        self.game.tableau[0] = [Card('♠', '6', face_up=True)]
        card = Card('♥', '5', face_up=True)
        self.assertTrue(self.game.is_valid_tableau_move(card, 0))

    def test_invalid_tableau_move_same_color(self):
        self.game.tableau[0] = [Card('♦', '6', face_up=True)]
        card = Card('♥', '5', face_up=True)
        self.assertFalse(self.game.is_valid_tableau_move(card, 0))

    def test_valid_foundation_move_empty_accepts_ace(self):
        card = Card('♣', 'A', face_up=True)
        self.assertTrue(self.game.is_valid_foundation_move(card, 3))

    def test_game_over_true_when_no_moves(self):
    ...
    def test_game_over_false_when_stock_exists(self):
        self.game.stock = [Card('♠', '2', face_up=False)]
        self.assertFalse(self.game.is_game_over())

    def test_game_over_false_with_empty_storage_and_face_up(self):
